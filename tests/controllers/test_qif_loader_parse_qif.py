...
def _mk_open(qif_text: str):
    ...
    @contextmanager
    def _fake_open(
        path, binary: bool = False, encoding: str = "utf-8", errors: str = "replace"
    ):
...
def test_parse_qif_account_block_sets_account_for_later_txns(monkeypatch):
    ...
    assert (
        t["account"] == "Checking"
    ), "Account list block should set current account for subsequent txns."
    assert t["type"] == "Bank"
...
def test_parse_qif_investment_action_vs_checknum(monkeypatch):
    ...
    assert (
        "checknum" not in t or t["checknum"] == ""
    ), "No checknum expected for investment records when 'N' is action."
...
def test_parse_qif_transfer_account_extracted_from_category(monkeypatch):
    ...
    assert (
        t["transfer_account"] == "Emergency Fund"
    ), "Transfer account should be parsed from bracketed category."
...
def test_parse_qif_finalize_on_eof_without_caret(monkeypatch):
    ...
    assert (
        len(txns) == 1
    ), "Parser should finalize the last record at EOF even without '^'."
    t = txns[0]
    assert t["payee"] == "Cafe"
...
def test_parse_qif_whitespace_and_unknown_lines_do_not_break(monkeypatch):
    ...
    assert (
        t["payee"] == "Payee With Space Prefix"
    ), "Leading whitespace should be stripped from field values."
...
def test_parse_qif_transfer_variants(monkeypatch, cat_line, expected_acct):
    ...
    assert t["category"].startswith("[") and t["category"].endswith(
        "]"
    ), "Literal bracketed category should be preserved."
    # Transfer account is normalized per regex variants
    assert (
        t.get("transfer_account") == expected_acct
    ), f"Expected transfer account '{expected_acct}' parsed from {cat_line.strip()!r}."

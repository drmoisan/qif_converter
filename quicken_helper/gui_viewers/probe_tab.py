...
class ProbeTab(ttk.Frame):
    ...
    def _build(self):
        pad = {'padx': 8, 'pady': 6}
        self.p_qdx = tk.StringVar(); self.p_qif = tk.StringVar(); self.p_out = tk.StringVar()

        files = ttk.LabelFrame(self, text="Files"); files.pack(fill="x", **pad)
        ...
        res = ttk.Frame(self); res.pack(fill="both", expand=True, **pad)
        left = ttk.LabelFrame(res, text="Report"); left.pack(side="left", fill="both", expand=True, padx=4, pady=4)
        self.p_report = tk.Text(left, wrap="word"); self.p_report.pack(fill="both", expand=True, padx=4, pady=4)

        right = ttk.LabelFrame(res, text="Artifacts (decompressed blobs)"); right.pack(side="left", fill="both", expand=True, padx=4, pady=4)
        self.p_artifacts = tk.Listbox(right, exportselection=False); self.p_artifacts.pack(fill="both", expand=True, padx=4, pady=(0,4))
        btns = ttk.Frame(right); btns.pack(fill="x", padx=4, pady=(0,4))
        ...
        prev = ttk.LabelFrame(right, text="Artifact Preview"); prev.pack(fill="both", expand=False, padx=4, pady=(0,4))
        self.p_preview = tk.Text(prev, height=12, wrap="word"); self.p_preview.pack(fill="both", expand=True, padx=4, pady=4)
        self.p_artifacts.bind("<Double-Button-1>", lambda e: self._p_preview_artifact())

    # pickers
    def _p_browse_qdx(self):
        p = filedialog.askopenfilename(title="Select QDX file", filetypes=[("QDX files","*.qdx"),("All files","*.*")])
        if p: self.p_qdx.set(p)
    def _p_browse_qif(self):
        p = filedialog.askopenfilename(title="Select QIF (optional)", filetypes=[("QIF files","*.qif"),("All files","*.*")])
    ...
    def _p_run_probe(self):
        try:
            ...
            if not qdx.exists():
                self.mb.showerror("Error", "Please pick a valid QDX file."); return
            ...
            self.p_report.delete("1.0", "end"); self.p_report.insert("end", report)
    ...
    def _p_selected_artifact(self) -> Optional[Path]:
        ...
        try: return Path(self.p_artifacts.get(sel[0]))
        except Exception: return None

    def _p_open_artifact_folder(self):
        p = self._p_selected_artifact()
        if not p:
            self.mb.showinfo("Info", "Select an artifact first."); return
        folder = p.parent
        try:
    ...
    def _p_preview_artifact(self):
        ...
        if not p or not p.exists():
            self.mb.showinfo("Info", "Select an existing artifact to preview."); return
        try:
            data = p.read_bytes()
        except Exception as e:
